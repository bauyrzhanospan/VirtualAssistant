...
memes = glob.glob('./data/reasons/[!create]*')
numb = 0
for meme in memes:
...
for meme in memes:
    name = str(meme)[15:]
    x_text_temp, y_temp = data_helpers.data_labels(memes, numb, meme)
    numb = numb + 1
    if k == 0:
        x_text = x_text_temp
        y = y_temp
    else:
        x_text = x_text + x_text_temp
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        print(str(x_train.shape[1]) + " is length of the sentence.")
