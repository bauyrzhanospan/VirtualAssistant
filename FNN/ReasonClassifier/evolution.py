    ...
    while x > 0.001:
        while y > 0.001:
            ...
            with open("log.txt", 'a') as out:
                ...
                out.write(delta1 + '\n')
            delta2 = m.train(X, y, hidden_neurons=Hn + x, alpha=(alpha + (y / 10)), epochs=100000, dropout=False,
                             dropout_percent=0.2)
            with open("log.txt", 'a') as out:
                delta1 = str(delta2) + ": " + str((Hn + x)) + " " + str((alpha + (y / 10)))
                out.write(delta2 + '\n')
            delta3 = m.train(X, y, hidden_neurons=Hn - x, alpha=(alpha - (y / 10)), epochs=100000, dropout=False,
                             dropout_percent=0.2)
            with open("log.txt", 'a') as out:
